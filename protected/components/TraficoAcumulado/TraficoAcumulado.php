...
class TraficoAcumulado{
	public static function ini_filtros()
    {
    	$con = Yii::app()->db;
        ...
        $varView['modelo'] = self::modelos($varView['modelo']);

        //provincia
    	$sqlModelos_nv = "SELECT distinct provincia from trafico_acumulado";
        $requestModelos_nv = $con->createCommand($sqlModelos_nv);
        $varView['provincia'] = $requestModelos_nv->queryAll();
        $varView['provincia'] = self::selectConstructor($varView['provincia'], 'provincia');

        //grupo
    	$sqlModelos_nv = "SELECT distinct grupo from trafico_acumulado";
        $requestModelos_nv = $con->createCommand($sqlModelos_nv);
        $varView['grupo'] = $requestModelos_nv->queryAll();
        $varView['grupo'] = self::selectConstructor($varView['grupo'], 'grupo');

        return $varView;
    }

    public static function ConcesionariosTA($where, $fecha1, $fecha2){
    	//concesionario
    	...
    	$con = Yii::app()->db;
    	$sql = "SELECT distinct concesionario from trafico_acumulado where ".$where." AND ((DATE(fecha) BETWEEN '".$fecha1[0]."' AND '".$fecha1[1]."') OR (DATE(fecha) BETWEEN '".$fecha2[0]."' AND '".$fecha2[1]."'))";
        $request = $con->createCommand($sql);
        $concesionarios = $request->queryAll();
        echo self::optionsConstructor($concesionarios, 'concesionario');
    }

    public static function optionsConstructor($array, $name){
    ...
    public static function modelos($modelos){
