...
class PvcodigoCausalController extends Controller
{
	/**
	...
	{
		$model=new CodigoCausal;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['CodigoCausal']))
		{
			$model->attributes=$_POST['CodigoCausal'];
			if($model->save()){
				Yii::app()->user->setFlash('success', '<img src="' . Yii::app()->request->baseUrl . '/images/agradecimiento.png"/>');
                $this->redirect(array('pvcodigoCausal/create'));
			}
		}
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['CodigoCausal']))
		{
			...
			if($model->save()){
				Yii::app()->user->setFlash('success', '<img src="' . Yii::app()->request->baseUrl . '/images/agradecimiento.png"/>');
                $this->redirect(array('pvcodigoCausal/update/'.$model->id));
			}
		}
	...
	public function actionSearch()
	{
		...
		if(!empty($_GET['Modelos']['Descripcion'])){
			...
			$posts = CodigoCausal::model()->findAll(array('order' => 'id DESC', 'condition' => "descripcion LIKE :match OR codigo LIKE :match", 'params' => array(':match' =>"%$patronBusqueda%")));
	...
	{
		$dataProvider=new CActiveDataProvider('CodigoCausal');
		...
		//	die();
			...
			switch (strtolower($extension[1])){
					   ...
					   default:
							...
							$this->redirect(array('pvcodigoCausal/adjunto/'));
							$error = 1;
							break;
			 }
			...
			if($error == 0) {
				...
				//die();
				if ($uploadedFile->saveAs(Yii::app()->basePath . '/../upload/codigocausal/' . $fileName)){
					try
					{
						$errorSave = "";
						$uploadfile = Yii::app()->basePath . '/../upload/codigocausal/' . $fileName;
	...
	public function actionAdmin()
	{
        ...
        // Count total records
        $pages = new CPagination(CodigoCausal::model()->count($criteria));
        ...
        // Grab the records
        $posts = CodigoCausal::model()->findAll($criteria);
	 ...
	 * @return CodigoCausal the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=CodigoCausal::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param CodigoCausal $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='codigo-causal-form')
		{
