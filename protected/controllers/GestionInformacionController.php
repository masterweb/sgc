...
//die('sql trafico: '.$sqlTrafico);
                $sqlTrafico2 = "SELECT nombres, apellidos, responsable, fecha from gestion_informacion 
WHERE responsable = {$id_responsable} AND DATE(fecha) BETWEEN '{$fecha_inicial_actual}' AND '{$fecha_actual}'";
//die('sql trafico2: '.$sqlTrafico2);
//echo 'sql trafico: trafico1: '.$sqlTrafico.', trafico2: '.$sqlTrafico2;
//die();
...
//            die();
            ...
            if ($_GET['GestionInformacion']['tipousuario'] == 1) {
                ...
                $titulo = 'Búsqueda entre ' . $fecha_inicial_actual_titulo . ' / ' . $fecha_actual_titulo . ', y ' . $fecha_inicial_anterior_titulo . ' / ' . $fecha_anterior_titulo;
                // BUSQUEDA POR TRAFICO
                $trafico_mes_anterior = $this->getSelect($cargo_id, 1, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $trafico_mes_actual = $this->getSelect($cargo_id, 1, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
                ...
                $traficocbu2 = $trafico_mes_actual - $traficockd2; // resto de modelos
                // BUSQUEDA POR PROFORMA
                $proforma_mes_anterior = $this->getSelect($cargo_id, 2, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $proforma_mes_actual = $this->getSelect($cargo_id, 2, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
                ...
                // BUSQUEDA POR TEST DRIVE
                $td_mes_anterior = $this->getSelect($cargo_id, 3, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $td_mes_actual = $this->getSelect($cargo_id, 3, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
                ...
                // BUSQUEDA POR VENTAS
                $vh_mes_anterior = $this->getSelect($cargo_id, 4, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $vh_mes_actual = $this->getSelect($cargo_id, 4, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
            ...
            if ($_GET['GestionInformacion']['tipousuario'] == 2 &&
                ...
                // BUSQUEDA POR TRAFICO
                $trafico_mes_anterior = $this->getSelect($cargo_id, 5, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $trafico_mes_actual = $this->getSelect($cargo_id, 5, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
                ...
                //
                // BUSQUEDA POR PROFORMA
                $proforma_mes_anterior = $this->getSelect($cargo_id, 6, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $proforma_mes_actual = $this->getSelect($cargo_id, 6, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
                ...
                // BUSQUEDA POR TEST DRIVE
                $td_mes_anterior = $this->getSelect($cargo_id, 7, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $td_mes_actual = $this->getSelect($cargo_id, 7, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
                ...
                // BUSQUEDA POR VENTAS
                $vh_mes_anterior = $this->getSelect($cargo_id, 8, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $vh_mes_actual = $this->getSelect($cargo_id, 8, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
            ...
            if ($_GET['GestionInformacion']['tipousuario'] == 3 &&
                ...
                // BUSQUEDA POR TRAFICO
                $trafico_mes_anterior = $this->getSelect($cargo_id, 9, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $trafico_mes_actual = $this->getSelect($cargo_id, 9, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
                ...
                $traficocbu2 = $trafico_mes_actual - $traficockd2; // resto de modelos
                // BUSQUEDA POR PROFORMA
                $proforma_mes_anterior = $this->getSelect($cargo_id, 10, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $proforma_mes_actual = $this->getSelect($cargo_id, 10, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
                ...
                // BUSQUEDA POR TEST DRIVE
                $td_mes_anterior = $this->getSelect($cargo_id, 11, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $td_mes_actual = $this->getSelect($cargo_id, 11, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
                ...
                // BUSQUEDA POR VENTAS
                $vh_mes_anterior = $this->getSelect($cargo_id, 12, $fecha_inicial_anterior, $fecha_anterior, $responsable, $concesionario);
                $vh_mes_actual = $this->getSelect($cargo_id, 12, $fecha_inicial_actual, $fecha_actual, $responsable, $concesionario);
...
GROUP BY gv.id_informacion";
                    ...
                    WHERE u.grupo_id = {$grupo_id} AND DATE(gi.fecha) BETWEEN '{$fecha1}' AND '{$fecha2}'";
                            ...
                            $sql = "SELECT gi.nombres, gi.apellidos, gi.responsable, gi.fecha from gestion_informacion gi
                                INNER JOIN usuarios u ON u.id = gi.responsable 
WHERE u.dealers_id = {$grupo_id} AND DATE(gi.fecha) BETWEEN '{$fecha1}' AND '{$fecha2}'";
                        }
                        //die($sql);
                        $requestr1 = $con->createCommand($sql);
                        $result = $requestr1->queryAll();
                    ...
                    case 10: // proforma con fecha y concesionario
                        ...
                        INNER JOIN gestion_informacion gi ON gi.id = gf.id_informacion 
                        INNER JOIN usuarios u ON u.id = gi.responsable
                    WHERE DATE(gf.fecha) BETWEEN '{$fecha1}' AND '{$fecha2}' AND u.grupo_id = {$grupo_id} GROUP BY gf.id_vehiculo  AND gf.order = 1 ";
                        } else {
                            $sql = "SELECT gf.id_informacion, gf.id_vehiculo, gf.fecha, gi.responsable, gi.dealer_id FROM gestion_financiamiento gf 
INNER JOIN gestion_informacion gi ON gi.id = gf.id_informacion
