...
class CencuestadoscquestionarioController extends Controller {
    ...
    public function actionGenerar($id) {
        ...
        $asesores = Usuarios::model()->findAll(array("condition" => "estado = 'ACTIVO'"));
    ...
    public function VerificarEncuestasAutomaticas($id) {
        ...
        if ($encuesta->automatico == 'SI') {
            ...
            if ($encuesta->cbasedatos_id == 1 && !empty($encuestados)) {
            ...
            if ($encuesta->cbasedatos_id == 2 && !empty($encuestados)) {
                //obtener todas las personas existentes en la base de datos TEST DRIVE
                $sql = 'SELECT DISTiNCT gi.nombres, gi.apellidos, gi.email, gi.celular, gi.telefono_oficina, gi.ciudad_conc
                    FROM gestion_informacion gi 
                        inner join gestion_nueva_cotizacion gt 
                    ...
                    if ($item->ctipopregunta_id == 3) {
                        ...
                        if (isset($_POST['respuesta'][$item->id])) {
                            ...
                            if (!empty($respuestasPreguntas['justifica'])) {
                                $model->respuesta = $p->purify($respuestasPreguntas['respuesta'] . ' por: ' . $respuestasPreguntas['justifica']);
                            } else {
                                $model->respuesta = $p->purify($respuestasPreguntas['respuesta']);
                            }
                        ...
                        if (!empty($_POST['respuesta'][$item->id])) {
                            ...
                            for ($i = 0; $i <= count($respuestasPreguntas); $i++) {
                                if (!empty($respuestasPreguntas[$i]) && $respuestasPreguntas[$i] != '') {
                                    ...
                                    $model->fecha = date('Y-m-d h:i:s');
                                    $model->pregunta_id = $item->id;
                                    $model->respuesta = $p->purify($respuestasPreguntas[$i]);
                                    //echo 'error-> '.$item->id.' - '.$respuestasPreguntas[$i].'<br>';
                                    if (!$model->save()) {
                                        $error++;
                                        echo 'error-> ' . $item->id . ' - ' . $respuestasPreguntas[$i] . '<br>';
                        ...
                        if (!empty($opcionesR)) {
                            foreach ($opcionesR as $op) {
                                ...
                                $model->fecha = date('Y-m-d h:i:s');
                                $model->pregunta_id = $item->id;
                                $model->respuesta = $p->purify($respuestasPreguntas[$op->id]);
                                if (!$model->save()) {
                                    $error++;
...
//die();
		...
		/* $sql = 'SELECT *
            ...
            if(!empty($datosC)){
				$v = Cotizacionesnodeseadas::model()->find(array('condition'=>'atencion_detalle_id='.$datosC[0]['id_atencion_detalle']));
				
                if(empty($v)){
					
					$to_time = strtotime($datosC[0]['fechaencuesta']);
					$from_time = strtotime(date('Y-m-d h:i:s'));
					if(round(abs($to_time - $from_time) / 60,2)>=10){
						
						Yii::app()->db2->createCommand("UPDATE atencion_detalle SET encuestado=0 WHERE  id_atencion_detalle =:RListID")->bindValues(array( ':RListID' => $datosC[0]['id_atencion_detalle']))->execute();
					}
				}
            }
        ...
        $datos = Yii::app()->db2->createCommand($sql)->queryAll();
        if(!empty($datos)){
            
            Yii::app()->db2
            ->createCommand("UPDATE atencion_detalle SET encuestado=2,fechaencuesta='".date("Y-m-d h:i:s")."' WHERE id_atencion_detalle=:RListID")
            ->bindValues(array( ':RListID' => $datos[0]['id_atencion_detalle']))
            ->execute();
        }/*else{
            $sql = 'SELECT *
            FROM atencion_detalle
            ...
            $datos = Yii::app()->db2->createCommand($sql)->queryAll();
            if(!empty($datos)){
                Yii::app()->db2->createCommand("UPDATE atencion_detalle SET encuestado=0 WHERE encuestado=2 and id_atencion_detalle!=:RListID")->bindValues(array( ':RListID' => $datos[0]['id_atencion_detalle']))->execute();
            }
        }*/
	...
	public function actionNocompradores($id){
		...
		if(!empty($_POST)){
			...
			if($noc->update()){
				...
				if(!empty($gd)){
					$gi = GestionInformacion::model()->findByPk($gd->id_informacion);
					if(trim($_POST['datos']['motivo'])== 'Falta de Dinero' && trim($_POST['datos']['compro'])=='NO'){
						//actualizamos el gestion de informacion
						if(!empty($gi)){
							$gi->tipo_form_web ='usado';
							if($gi->update()){
								$gestion = new GestionDiaria();
			...
			/*$modelC = GestionDiaria::model()->find(array('condition'=>'desiste = 1 and encuestado=2'));
			if(!empty($modelC)){
				$to_time = strtotime($modelC->realizado);
				$from_time = strtotime(date('Y-m-d h:i:s'));
				if(round(abs($to_time - $from_time) / 60,2)>=10){
					$modelC->encuestado ='0';
					$modelC->update();
			...
			$model = GestionDiaria::model()->find(array('condition'=>'desiste = 1 and encuestado=0'));
			if(!empty($model)){
				$model->encuestado = 2;
				$model->realizado = date('Y-m-d h:i:s');
				$model->update();
			}else{
				$modelD = GestionDiaria::model()->findAll(array('condition'=>'desiste = 1 and encuestado=2'));
				if(!empty($modelD)){
					if(GestionDiaria::model()->updateAll(array('encuestado'=>0),array('condition'=>'desiste = 1 and encuestado=2'))){
						$model = GestionDiaria::model()->find(array('condition'=>'desiste = 1 and encuestado=0'));
					}
				}
			}*/
	...
	public function actionAtenciondetalle(){
		date_default_timezone_set("America/Bogota");
		/*$sql = 'SELECT * FROM atencion_detalle WHERE encuestado = 0 limit 100';
        $datosC = Yii::app()->db2->createCommand($sql)->queryAll();
