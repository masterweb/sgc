            ...
            <div class="highlight">
                <div class="form">
                    ...
                    $form = $this->beginWidget('CActiveForm', array(
                        'id' => 'Qir-form',
                        'method' => 'get',
                        'action' => Yii::app()->createUrl('pvQirfiles/search/id/' . $id),
                        'enableAjaxValidation' => true,
                        'htmlOptions' => array('class' => 'form-horizontal form-search')
                    ));
                    ?>
                    <label class="col-sm-4 control-label" for="Casos_estado">Busqueda Adjunto</label>
                    <div class="col-md-5">
                        <?php echo $form->textField($modelPost, 'nombre', array('class' => "form-control")) ?>                        
        ...
        <div class="table-responsive">
			...
			<?php if(!empty($accesosUser)){
					foreach($accesosUser as $a){
						if( ($a->accesoSistema->controlador) == 'pvQirfiles' &&  ($a->accesoSistema->accion) ==  'create'){?>	
							<a href="<?php echo Yii::app()->createUrl('pvQirfiles/create/id/' . $id); ?>" class="btn btn-primary btn-xs btn-danger btnCrear">Crear Adjunto</a>                
					<?php }
					}
				}?>
            </div>
                ...
                <tbody>
                    ...
                    if ($model) {
                        foreach ($model as $c):
                            ...
                            <tr>
                                <td><?php echo $c->id; ?> </td>
                                <td><a href="<?php echo Yii::app()->request->baseUrl; ?>/upload/qirfiles/<?php echo $c->nombre ?>" target="_blank"><?php echo $c->nombre ?></a></td>
                                <td><?php echo $c->num_reporte ?> </td>
								
								 <?php if(!empty($accesosUser)){
									foreach($accesosUser as $a){
									?>
									<?php if( ($a->accesoSistema->controlador) == 'pvQirfiles' &&  ($a->accesoSistema->accion) ==  'view'){?>	
										<td><a href="<?php echo Yii::app()->createUrl('pvQirfiles/view', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Ver</a></td>
									<?php } ?>
										
									<?php if( ($a->accesoSistema->controlador) == 'pvQirfiles' &&  ($a->accesoSistema->accion) ==  'update'){?>	     
										<td><a href="<?php echo Yii::app()->createUrl('pvQirfiles/update', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Editar</a></td>
									<?php } ?>
										
									<?php if( ($a->accesoSistema->controlador) == 'pvQirfiles' &&  ($a->accesoSistema->accion) ==  'eliminar'){?>	     
										<td><?php echo CHtml::link('Eliminar', array('pvQirfiles/eliminar', 'id' => $c->id), array('class' => 'btn btn-primary btn-xs btn-danger', 'onclick' => "return confirm('Esta seguro que desea eliminar este Adjunto?')")) ?>
									<?php } 
