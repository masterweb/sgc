...
<div class="container">
    <div class="row">
        <h1 class="tl_seccion">C&oacute;digo Naturaleza</h1>
    </div>
	<div class="row">
            ...
            <div class="highlight">
                <div class="form">
                    ...
                    $form = $this->beginWidget('CActiveForm', array(
                        ...
                        'method' => 'get',
                        'action' => Yii::app()->createUrl('pvcodigoNaturaleza/search'),
                        'enableAjaxValidation' => true,
                        'htmlOptions' => array('class' => 'form-horizontal form-search')
                            ));
                    ?>
                    <label class="col-sm-2 control-label" for="Casos_estado">C&oacute;digo o Descripci&oacute;n</label>
                        <div class="col-md-6">
		...
		<?php 
			if(!empty($accesosUser)){
				foreach($accesosUser as $a){
					if( ($a->accesoSistema->controlador) == 'pvcodigoNaturaleza' &&  ($a->accesoSistema->accion) ==  'create'){?>	
						<a href="<?php echo Yii::app()->createUrl('pvcodigoNaturaleza/create'); ?>" class="btn btn-primary btn-xs btn-danger btnCrear">Crear C&oacute;digo Naturaleza</a>
					<?php } 
					if( ($a->accesoSistema->controlador) == 'pvcodigoNaturaleza' &&  ($a->accesoSistema->accion) ==  'adjunto'){?>
						<a href="<?php echo Yii::app()->createUrl('pvcodigoNaturaleza/adjunto'); ?>" class="btn btn-primary btn-xs btn-danger btnCrear">Cargar Excel</a>
					<?php } ?>
			<?php
				}
			}
                        ...
                        <tr>
							...
							<?php 
								if(!empty($accesosUser)){
									foreach($accesosUser as $a){
							?>
							<?php if( ($a->accesoSistema->controlador) == 'pvcodigoNaturaleza' &&  ($a->accesoSistema->accion) ==  'update'){?>	
								<td><a href="<?php echo Yii::app()->createUrl('pvcodigoNaturaleza/update', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Editar</a></td>
							<?php } ?>
							
							<?php if( ($a->accesoSistema->controlador) == 'pvcodigoNaturaleza' &&  ($a->accesoSistema->accion) ==  'eliminar'){?>	
								<td><a href="<?php echo Yii::app()->createUrl('pvcodigoNaturaleza/eliminar', array('id' => $c->id)); ?>" onclick="return confirm('&iquest;Esta seguro que desea eliminar este C&oacute;digo Naturaleza?')" class="btn btn-primary btn-xs btn-danger">Eliminar</a></td>
							<?php } ?>
														
							<?php if( ($a->accesoSistema->controlador) == 'pvcodigoNaturaleza' &&  ($a->accesoSistema->accion) ==  'view'){?>	
									<td><a href="<?php echo Yii::app()->createUrl('pvcodigoNaturaleza/view', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Ver</a></td>
							<?php } 
