...
<div class="container">
    <div class="row">
        <h1 class="tl_seccion">QIR Comentario adjuntos</h1>
    </div>
    <div class="row">
            ...
            <div class="highlight">
                <div class="form">
                    ...
                    $form = $this->beginWidget('CActiveForm', array(
                        'id' => 'Qir-form',
                        'method' => 'get',
                        'action' => Yii::app()->createUrl('pvQirComentarioFile/search/id/' . $id),
                        'enableAjaxValidation' => true,
                        'htmlOptions' => array('class' => 'form-horizontal form-search')
                            ));
                    ?>
                    <label class="col-sm-4 control-label" for="Casos_estado">Busqueda documento Adjunto</label>
                    <div class="col-md-5">
                        <?php echo $form->textField($modelPost, 'nombre_file', array('class' => "form-control")) ?>                        
        ...
        <div class="col-md-3 col-md-offset-1 cont_der">
            ...
            <ul>
                <li><a href="<?php echo Yii::app()->createUrl('pvQircomentario/admin/id/' . $modelQir->qirId); ?>" class="seguimiento-btn">Administrador de comentarios</a></li>
        ...
        <div class="table-responsive">
			...
			<?php if(!empty($accesosUser)){
						foreach($accesosUser as $a){
							if( ($a->accesoSistema->controlador) == 'pvQirComentarioFile' &&  ($a->accesoSistema->accion) ==  'create'){?>	
								<a href="<?php echo Yii::app()->createUrl('pvQirComentarioFile/create/id/' . $id); ?>" class="btn btn-primary btn-xs btn-danger btnCrear">Crear documento adjunto</a>                
				<?php }
						}
					}?>
            </div>
                ...
                <tbody>
                    ...
                    if ($model) {
                        foreach ($model as $c):
                            ...
                            <tr>
                                <td><?php echo $c->id; ?> </td>
                                <td><a href="<?php echo Yii::app()->request->baseUrl; ?>/upload/qircomentariofile/<?php echo $c->nombre_file ?>" target="_blank"><?php echo $c->nombre_file ?></a></td>
                               <?php if(!empty($accesosUser)){
									foreach($accesosUser as $a){
									?>
									<?php if( ($a->accesoSistema->controlador) == 'pvQirComentarioFile' &&  ($a->accesoSistema->accion) ==  'view'){?>	
										<td><a href="<?php echo Yii::app()->createUrl('pvQirComentarioFile/view', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Ver</a></td>
									<?php } ?>
										
									<?php if( ($a->accesoSistema->controlador) == 'pvQirComentarioFile' &&  ($a->accesoSistema->accion) ==  'update'){?>	                           
										<td><a href="<?php echo Yii::app()->createUrl('pvQirComentarioFile/update', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Editar</a></td>
									<?php } ?>
										
									<?php if( ($a->accesoSistema->controlador) == 'pvQirComentarioFile' &&  ($a->accesoSistema->accion) ==  'eliminar'){?>	                           
										<td><?php echo CHtml::link('Eliminar', array('pvQirComentarioFile/eliminar', 'id' => $c->id), array('class' => 'btn btn-primary btn-xs btn-danger', 'onclick' => "return confirm('Esta seguro que desea eliminar este Adjunto?')")) ?>
									<?php } 
    ...
    <div class="col-md-8">
        <h1 class="tl_seccion">Ver Comentario #<?php echo $modelQir->id; ?></h1>
        <?php
        ...
        $this->widget('zii.widgets.CDetailView', array(
            ...
            'attributes' => array(
                ...
                array(
                    'name' => 'descripcion',
                    'label' => 'Descripcion',
                    'value' => $modelQir->qir->descripcion,
                    'type' => 'raw'
                ),
                'ingresado',
                'email',
