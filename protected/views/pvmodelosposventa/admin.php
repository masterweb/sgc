...
<div class="container">
    <div class="row">
        <h1 class="tl_seccion">Modelos PosVenta</h1>
    </div>
	<div class="row">
            ...
            <div class="highlight">
                <div class="form">
                    ...
                    $form = $this->beginWidget('CActiveForm', array(
                        ...
                        'method' => 'get',
                        'action' => Yii::app()->createUrl('pvmodelosposventa/search'),
		...
		<?php 
			if(!empty($accesosUser)){
				foreach($accesosUser as $a){
					if( ($a->accesoSistema->controlador) == 'pvmodelosposventa' &&  ($a->accesoSistema->accion) ==  'create'){?>	
						<a href="<?php echo Yii::app()->createUrl('pvmodelosposventa/create'); ?>" class="btn btn-primary btn-xs btn-danger btnCrear">Crear Modelo PosVenta</a>
				<?php }
			}
		}?>
		</div>
...
//$model = Casos::model()->findAll();
					...
					<?php 
						if(!empty($accesosUser)){
							foreach($accesosUser as $a){
							?>
								<?php if( ($a->accesoSistema->controlador) == 'pvmodelosposventa' &&  ($a->accesoSistema->accion) ==  'update'){?>	
									<td><a href="<?php echo Yii::app()->createUrl('pvmodelosposventa/update', array('id' => $c['id'])); ?>" class="btn btn-primary btn-xs btn-danger">Editar</a></td>
								<?php } ?>
								
								<?php if( ($a->accesoSistema->controlador) == 'pvmodelosposventa' &&  ($a->accesoSistema->accion) ==  'eliminar'){?>	                           
									<td><a href="<?php echo Yii::app()->createUrl('pvmodelosposventa/eliminar', array('id' => $c->id)); ?>" onclick="return confirm('&iquest;Esta seguro que desea eliminar este modelo?')" class="btn btn-primary btn-xs btn-danger">Eliminar</a></td>
								<?php } ?>
																
									<?php if( ($a->accesoSistema->controlador) == 'pvmodelosposventa' &&  ($a->accesoSistema->accion) ==  'view'){?>	
										<td><a href="<?php echo Yii::app()->createUrl('pvmodelosposventa/view', array('id' => $c['id'])); ?>" class="btn btn-primary btn-xs btn-danger">Ver</a></td>
								<?php } 
