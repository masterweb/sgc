                    ...
                    <tbody>
                    	<?php foreach ($exh as $value) { ?>
                    		<tr>
                    			...
                    			<td><?php 
                                    $id_editor = $this->getResponsableEditSc($value['id'], $value['id_vehiculo']);
                                    if(!empty($id_editor)){
                    			...
                    			<td><?php echo $this->getResponsable($value['responsable']); ?></td>
                    			<td><?php echo $this->getConcesionario($value['dealer_id']); ?></td>
                    			<td>
                    				<?php
                                    $status = $this->getStatusSolicitud($value['id'], $value['id_vehiculo']);
                                    //echo 'status: '.$status;
                                    switch ($status) {
                                        case 'na':
                                ...
                                <td>
                                    ...
                                    if($sc > 0):
                                    ?>
                                    <a href="<?php echo Yii::app()->createUrl('gestionSolicitudCredito/update/', array('id_informacion' => $value['id'], 'id_vehiculo' => $value['id_vehiculo'])); ?>" class="btn btn-primary btn-xs btn-danger">Editar</a>
                                    <?php else: ?>
                                    <a href="<?php echo Yii::app()->createUrl('gestionSolicitudCredito/create/', array('id_informacion' => $value['id'], 'id_vehiculo' => $value['id_vehiculo'])); ?>" class="btn btn-primary btn-xs btn-danger">Ingresar datos</a>
                                	<?php endif; ?>
                                </td>
                                <td>
                                	<?php
                                    $firma = GestionFirma::model()->count(array('condition' => "id_informacion = {$value['id']}"));
                                    if($firma > 0){
