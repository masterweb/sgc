...
<div class="container">
    <div class="row">
        <h1 class="tl_seccion">QIR Comentarios</h1>
    </div>
    <div class="row">
            ...
            <div class="highlight">
                <div class="form">
                    ...
                    $form = $this->beginWidget('CActiveForm', array(
                        'id' => 'Qir-form',
                        'method' => 'get',
                        'action' => Yii::app()->createUrl('pvQircomentario/search/id/'.$id),
                        'enableAjaxValidation' => true,
                        'htmlOptions' => array('class' => 'form-horizontal form-search')
                            ));
                    ?>
                    <label class="col-sm-4 control-label" for="Casos_estado">Buscar en Qir Comentario</label>
                    <div class="col-md-5">
                        <?php echo $form->textField($modelPost,'estado',array('class'=>"form-control"))?>                        
            ...
            <div>
				<?php if(!empty($accesosUser)){
						foreach($accesosUser as $a){
							if( ($a->accesoSistema->controlador) == 'pvQircomentario' &&  ($a->accesoSistema->accion) ==  'create'){?>	
								<a href="<?php echo Yii::app()->createUrl('pvQircomentario/create/id/'.$id); ?>" class="btn btn-primary btn-xs btn-danger btnCrear">Agregar Comentario</a>                
						<?php }
						}
					}?>
            </div>
                        ...
                        foreach ($model as $c):
                            ...
                            <tr>
								...
								<?php if(!empty($accesosUser)){
									foreach($accesosUser as $a){
									?>
										<?php if( ($a->accesoSistema->controlador) == 'pvQircomentario' &&  ($a->accesoSistema->accion) ==  'view'){?>	
											<td><a href="<?php echo Yii::app()->createUrl('pvQircomentario/view', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Ver</a></td>
										<?php } ?>
										
										<?php if( ($a->accesoSistema->controlador) == 'pvQircomentario' &&  ($a->accesoSistema->accion) ==  'update'){?>	                           
											<td><a href="<?php echo Yii::app()->createUrl('pvQircomentario/update', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Editar</a></td>
										<?php } ?>
										
										<?php if( ($a->accesoSistema->controlador) == 'pvQircomentario' &&  ($a->accesoSistema->accion) ==  'eliminar'){?>	                           
											<td><?php echo CHtml::link('Eliminar', array('pvQircomentario/eliminar', 'id' => $c->id), array('class' => 'btn btn-primary btn-xs btn-danger', 'onclick' => "return confirm('Esta seguro que desea eliminar este Comentario?')")) ?>
										<?php } ?>
										
										<?php if( ($a->accesoSistema->controlador) == 'pvQirComentarioFile' &&  ($a->accesoSistema->accion) ==  'admin'){?>	                           
											<td><?php echo CHtml::link('Adjuntar Archivo', array('pvQirComentarioFile/admin', 'id' => $c->id), array('class' => 'btn btn-primary btn-xs btn-danger')) ?>
										<?php } 
