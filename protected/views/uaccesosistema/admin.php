...
<div class="container">
    <div class="row">
        <h1 class="tl_seccion">Accesos del Sistema</h1>
    </div>
	<div class="row">
            ...
            <div class="highlight">
                <div class="form">
                    ...
                    $form = $this->beginWidget('CActiveForm', array(
                        'id' => 'search-form',
                        'method' => 'get',
                        'action' => Yii::app()->createUrl('uaccesosistema/search'),
    ...
    <div class="row">
		...
		<?php if(!empty($accesosUser)){
					foreach($accesosUser as $a){
						if( ($a->accesoSistema->controlador) == 'uaccesosistema' &&  ($a->accesoSistema->accion) ==  'create'){?>	
							<a href="<?php echo Yii::app()->createUrl('uaccesosistema/create'); ?>" class="btn btn-primary btn-xs btn-danger btnCrear">Crear Acceso</a>
					<?php }
					}
				}?>
		</div>
...
//$model = Casos::model()->findAll();
                    ...
                    foreach ($model as $c):
                        ...
                        <tr>
                            <td><?php echo $cont; ?> </td>
							 <td><?php echo $c->modulo->descripcion ?> </td>
                            <td><?php echo $c->descricion ?> </td>
                           
                            <td><?php echo $c->accion ?> </td>
                            <td><?php echo $c->estado ?> </td>
							
							 <?php if(!empty($accesosUser)){
									foreach($accesosUser as $a){
									?>
									<?php if( ($a->accesoSistema->controlador) == 'uaccesosistema' &&  ($a->accesoSistema->accion) ==  'update'){?>	
										<td><a href="<?php echo Yii::app()->createUrl('uaccesosistema/update', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Editar</a></td>
									<?php } ?>
										
									<?php if( ($a->accesoSistema->controlador) == 'uaccesosistema' &&  ($a->accesoSistema->accion) ==  'eliminar'){?>	     
									   <td><a href="<?php echo Yii::app()->createUrl('uaccesosistema/eliminar', array('id' => $c->id)); ?>" onclick="return confirm('&iquest;Esta seguro que desea eliminar este Acceso?')" class="btn btn-primary btn-xs btn-danger">Eliminar</a></td>
									<?php } ?>
										
									<?php if( ($a->accesoSistema->controlador) == 'uaccesosistema' &&  ($a->accesoSistema->accion) ==  'view'){?>	     
										<td><a href="<?php echo Yii::app()->createUrl('uaccesosistema/view', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Ver</a></td>
									<?php } 
