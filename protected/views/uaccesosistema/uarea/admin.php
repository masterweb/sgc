...
<div class="container">
    <div class="row">
        <h1 class="tl_seccion">&Aacute;reas</h1>
    </div>
	<div class="row">
            ...
            <div class="highlight">
                <div class="form">
                    ...
                    $form = $this->beginWidget('CActiveForm', array(
                        'id' => 'search-form',
                        'method' => 'get',
                        'action' => Yii::app()->createUrl('uarea/search'),
    ...
    <div class="row">
		...
		<?php if(!empty($accesosUser)){
					foreach($accesosUser as $a){
						if( ($a->accesoSistema->controlador) == 'uarea' &&  ($a->accesoSistema->accion) ==  'create'){?>
			<a href="<?php echo Yii::app()->createUrl('uarea/create'); ?>" class="btn btn-primary btn-xs btn-danger btnCrear">Crear &Aacute;rea</a>
			<?php }
					}
				}?>
		</div>
...
//$model = Casos::model()->findAll();
                    ...
                    foreach ($model as $c):
                        ...
                        <tr>
                            ...
                            <td><?php echo ($c->tipo==1)?'AEKIA':'CONCESIONARIO'; ?> </td>
                            <td><?php echo $c->descripcion ?> </td>
							 <?php if(!empty($accesosUser)){
									foreach($accesosUser as $a){
									?>
									<?php if( ($a->accesoSistema->controlador) == 'uarea' &&  ($a->accesoSistema->accion) ==  'update'){?>	
										<td><a href="<?php echo Yii::app()->createUrl('uarea/update', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Editar</a></td>
   									<?php } ?>
										
									<?php if( ($a->accesoSistema->controlador) == 'uarea' &&  ($a->accesoSistema->accion) ==  'eliminar'){?>	     
										<td><a href="<?php echo Yii::app()->createUrl('uarea/eliminar', array('id' => $c->id)); ?>" onclick="return confirm('&iquest;Esta seguro que desea eliminar este elemento?')" class="btn btn-primary btn-xs btn-danger">Eliminar</a></td>
									<?php } ?>
										
									<?php if( ($a->accesoSistema->controlador) == 'uarea' &&  ($a->accesoSistema->accion) ==  'view'){?>	     
										<td><a href="<?php echo Yii::app()->createUrl('uarea/view', array('id' => $c->id)); ?>" class="btn btn-primary btn-xs btn-danger">Ver</a></td>
									<?php } ?>
