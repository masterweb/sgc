...
class Atencion {
    ...
    protected static $db_fields = array('id_atencion_detalle', 'nombre', 'apellido',
        'cedula', 'direccion', 'telefono', 'celular', 'email', 'id_modelos', 'id_version',
        'horario', 'id_provincia', 'cityid', 'dealerid', 'trabajo_solicitado',
        'modelo_vehiculo', 'numero_chasis', 'obs_requerim_comen',
        'ocupacion', 'id_atencion', 'fecha_form', 'ips',
        'motivo', 'tema', 'otro', 'id_origen',
    );
    public $id_atencion_detalle;
    public $nombre;
    public $apellido;
    public $cedula;
    public $direccion;
    ...
    public function getModels($id_modelo) {
        global $database;
        $sql = "SELECT id_versiones, nombre_version FROM versiones WHERE id_modelos = {$id_modelo} ORDER BY nombre_version ASC";
        $result = $database->getQuery($sql);
        $html = '<option value="">--Escoja un Modelo--</option>';
...
//                and a.dealerid = {$dealerid} AND e.fecha >= '{$fecha}'";
        ...
        $arrayDatos = array();
        $data = '<form action="http://www.kia.com.ec/administrador/ficheroExcel.php" method="post" target="_blank" id="FormularioExportacion">';
...
//        echo '</pre>';die();
    ...
    public function updateLinkActivo($dealerid) {
        global $database;
